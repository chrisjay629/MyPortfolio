...
/* ---- Ambient floating-node network (subtle starfield) ---- */
(function () {
  "use strict";
  ...
  function init() {
    var count = Math.max(24, Math.min(92, Math.floor((w * h) / 18000)));
    nodes = [];
  ...
  function frame() {
    ...
    for (i = 0; i < nodes.length; i++) {
      for (j = i + 1; j < nodes.length; j++) {
        ...
        if (d < MAX_DIST) {
          ctx.strokeStyle = "rgba(120,200,150," + ((1 - d / MAX_DIST) * 0.09).toFixed(3) + ")";
          ctx.lineWidth = 1;
          ctx.beginPath();
    ...
    for (i = 0; i < nodes.length; i++) {
      a = nodes[i];
      ctx.fillStyle = "rgba(206,232,214," + a.a.toFixed(2) + ")";
      ctx.beginPath();
